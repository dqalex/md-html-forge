 ...
 *
 * 注意：此文件已从 markdown-slots/ 移入 builtin/ 以消除循环依赖：
 *   - builtin/compiler/emitter → markdown-slots/slot-sync (MD→HTML 渲染)
 *   - markdown-slots/fallback-renderer → builtin/compiler (编译器)
 * 移入 builtin 后，方向统一为：builtin → markdown-slots，无循环。
 */

import { syncMdToHtml, extractSlotsFromMd, simpleMdToHtml, sanitizeHtml, inlineMdToHtml } from '@/lib/markdown-slots/slot-sync';
import { SHARED_TOKENS_CSS } from './components/shared-tokens';
import { parseComposeDirective } from './renderer';
import { compile } from './compiler';
import { forgeRegistry } from './compiler/registry';
import './bootstrap'; // 确保 registry 已初始化
...
export { parseComposeDirective } from './renderer';
